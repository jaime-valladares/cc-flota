...
<section class="cc-card cc-analytics-root-card">
    ...
    @else
        <div class="cc-result-count">Mostrando <strong class="cc-result-count-value">{{ $ciclos->firstItem() }}</strong>-<strong class="cc-result-count-value">{{ $ciclos->lastItem() }}</strong> de <strong class="cc-result-count-value">{{ $ciclos->total() }}</strong></div>
        <div class="cc-table-adaptive-wrapper"><table class="cc-table-adaptive cc-analytics-table-performance"><thead><tr><th>Empresa</th><th>Nombre / Placa</th><th>Motorista</th><th>Periodo</th><th>Km recorridos</th><th>Rendimiento</th><th>Consumo</th><th>Diferencia</th><th>Resultado</th><th>Impacto económico</th><th>Acción</th></tr></thead><tbody>
            @foreach($ciclos as $ciclo)<tr data-report-cycle-row="{{ $ciclo->id }}">
                ...
                <td class="whitespace-nowrap"><span class="cc-table-adaptive-muted">Inicio</span> {{ $ciclo->abastecimientoAnterior->fecha_hora_abastecimiento->format('d/m/Y H:i') }}<br><span class="cc-table-adaptive-muted">Cierre</span> {{ $ciclo->fecha_hora_abastecimiento->format('d/m/Y H:i') }}</td>
                <td class="text-right">{{ number_format((float)$ciclo->diferencia_kilometraje,2) }} km</td>
                <td class="text-right"><span class="cc-table-adaptive-muted">Teórico</span> {{ number_format((float)$ciclo->rendimiento_teorico_km_galon_snapshot,2) }}<br><span class="cc-table-adaptive-muted">Real</span> {{ number_format((float)$ciclo->kilometros_por_galon,2) }} km/gal</td>
                <td class="text-right"><span class="cc-table-adaptive-muted">Teórico</span> {{ number_format((float)$ciclo->consumo_teorico_ciclo,2) }}<br><span class="cc-table-adaptive-muted">Real</span> {{ number_format((float)$ciclo->consumo_real_ciclo,2) }} gal</td>
                ...
                <td class="text-right"><span class="cc-table-adaptive-strong" @class(['text-[var(--cc-success)]'=>$ciclo->resultado_reporte==='Ahorro','text-[var(--cc-danger)]'=>$ciclo->resultado_reporte==='Sobreconsumo'])>{{ $ciclo->resultado_reporte==='Sobreconsumo'?'-':($ciclo->resultado_reporte==='Ahorro'?'+':'') }}${{ number_format($ciclo->impacto_economico_reporte,2) }}</span></td>
